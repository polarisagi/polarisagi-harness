...
var cliDict = map[string][2]string{
	...
	"help_desc_init":  {"  polaris init             交互式初始化向导（配置 LLM 厂商 / 模型 / 渠道）", "  polaris init             Interactive setup wizard (LLM providers / models / channels)"},
	...
	"init_s3_title": {"步骤 3/3 · 消息渠道（可选）", "Step 3/3 · Messaging Channels (Optional)"},
	"init_s3_desc":  {"  接入 Telegram / 飞书等平台。不配置直接回车跳过。\n", "  Connect Telegram / Slack etc. Press Enter to skip.\n"},
	"init_c_q":      {"  现在配置消息渠道？", "  Configure a channel now?"},
	"init_c_opts":   {"  1) Telegram  2) 飞书(Feishu)  3) Slack  4) Discord  5) Webhook", "  1) Telegram  2) Feishu  3) Slack  4) Discord  5) Webhook"},
	"init_c_type":   {"  渠道类型 [1-5]", "  Channel Type [1-5]"},
	"init_c_name":   {"  渠道名称", "  Channel Name"},
	"init_c_fail":   {"  ✗ 保存失败: %s", "  ✗ Failed to save: %s"},
	"init_c_ok":     {"  ✓ 渠道已配置", "  ✓ Channel configured"},
