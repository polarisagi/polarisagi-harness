...
> **§跳读**: 0-bis:6 职责 / 0-ter:17 不变量速查 / 1:28 接口层 / 2:65 EventLog / 2.6:167 tasks表 / 3:202 容量 / 4:245 Workspace / 5:303 SchemaManager / 6:336 Reindexer / 7:370 Go↔Rust FFI / 8:394 连接池 / 9:402 多写者 / 10:413 引擎速查 / 11:422 四层记忆映射 / 15:428 428(SOFT)降级 / 16:440 依赖
## 0-bis. 职责边界
...
[Source-of-Truth: internal/protocol/interfaces.go # Store]
为彻底杜绝架构文档与代码接口签名发生漂移，所有方法签名（如 Get, Put, Delete, BatchWrite 等）以 `internal/protocol/interfaces.go` 中 `Store` 及 `Transaction` 接口定义为唯一权威。所有存储引擎适配器必须实现该接口以保证原子性封装与架构层契约的强制一致。
...
- `CompositeMutationIntent`: 全成功或全失败，GroupID 收集同组成员（最多等 2×ticker=20ms）
- `DatabaseWriter`: ch cap=4096, batch cap=64, ticker 10ms, MaxRowsPerTx=50
...
| Warm | 7 天 | SQLite events 表 | 全字段索引查询（低优先级） | 7 天后标记 `archived` 软删除 |
| Cold | 永久归档 | Parquet 文件（`data/cold/events/`） | 仅 DuckDB 回读 | >30 天 + 磁盘水位 <20% 触发 |
...
- 层: L0 | 机制: EventWriteBuffer MPSC 批量缓冲 | 阈值: channel 4096, batch 64, 100ms flush
- 层: L1 | 机制: PRAGMA busy_timeout | 阈值: 1000ms
- 层: L2 | 机制: PRAGMA wal_autocheckpoint | 阈值: 100 页
...
- 引擎: **[Storage-SurrealDB-Core]** (Rust cdylib via purego, CGO-Free FFI)
  - 用途: 认知检索轴。统一多模态认知脑回。内置 SurrealDB-Core KV + HNSW 向量索引 + CSR 知识图谱 + FTS 全文检索。一次性替代了过去的 LanceDB、CozoDB 和 Tantivy，解决内存抢占与多引擎碎片化，单栈闭环提供 process_turn 级别的 Agent 原语。
...
完整阈值与重评触发条件: `spec/state.yaml §thresholds.m2_storage`。最终值落 `config/m02.toml`。
