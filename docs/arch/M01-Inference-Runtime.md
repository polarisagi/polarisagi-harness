...
| L2 超时 | ~5ms |
| L3 超时 | ~50ms |
| CircuitBreaker 连续失败熔断阈值 | 5 |
| CircuitBreaker 冷却时间 | 10s |
| CircuitBreaker 半开探测上限 | 1 |
| MaxStreamBufferSize | 256KB (Tier 1+ 可配至 1MB) |
...
缓存设计意图（SurrealDB-Core HNSW 可用后实现）: SimilarityThreshold=0.95, MaxEntries=10000, TTL=24h。三重匹配: RequestHash + Namespace + SystemPromptHash。hashRequest: SHA-256(Namespace + SystemPromptHash + ContextHint.Fingerprint + ActiveControlVectorLabels + TaskType + MessageContents)。满时 LRU 淘汰 MaxEntries/10。
...
字段: failureThreshold=5, cooldownPeriod=10s, halfOpenMax=1
转换: 5 次连续失败 → Open (10s 冷却) → HalfOpen → 1 次探测成功 → Closed
```
...
完整阈值与重评触发条件: `spec/state.yaml §thresholds.m1_router`。最终值落 `config/m01.toml`。
